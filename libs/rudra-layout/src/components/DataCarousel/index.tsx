...
export interface RepeaterCarouselProps {
  ...
  template?: React.ReactNode | ((payload: any) => React.ReactNode); /* @nodeFunction */
...
const RepeaterCarousel: React.FC<RepeaterCarouselProps> = ({
  ...
  const scrollByAmount = (direction: 'left' | 'right') => {
    if (!scrollContainerRef.current) return;
    // Scroll by the width of the container for a full "page" turn
      ...
      */}
      ...
      >
        {items.map((item, index) => (
          ...
          >
            {/* Execute the template function provided by Rudra's engine */}
            {typeof template === "function" ? template({ item, index, _builderIndex: index }) : template}
          </div>
        ))}
      </div>
